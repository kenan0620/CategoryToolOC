...
+ (NSString *)uniqueUUIDWithKey:(NSString *)key {
    NSString * uniqueUUID = (NSString *)[KeyChain load:key];
    //首次执行该方法时，uuid为空
    if (!uniqueUUID || [uniqueUUID isEqualToString:@""]) {
        //生成一个uuid的方法
        CFUUIDRef uuidRef = CFUUIDCreate(kCFAllocatorDefault);
        uniqueUUID = (NSString *)CFBridgingRelease(CFUUIDCreateString (kCFAllocatorDefault,uuidRef));
        //将该uuid保存到keychain
        [KeyChain save:key data:uniqueUUID];
    }
    return uniqueUUID;
}
...
- (BOOL)isBangScreen {
    ...
    if (@available(iOS 11.0, *)) {
        result = ([[UIApplication sharedApplication] delegate].window.safeAreaInsets.bottom > 0.0);
    }
